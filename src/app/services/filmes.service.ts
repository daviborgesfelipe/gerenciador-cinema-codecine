...
export class FilmeService implements OnInit{
  ...
  constructor(private http: HttpClient) {
    this.http.get<any>(`${this.NOTAS_API_URL}?language=pt-BR&page=1`, this.obterHeaderAutorizacao() )
  ...
  selecionarFilmes(): Observable<Filme[]> {
    return this.http.get<any>(
      `${this.NOTAS_API_URL}?language=pt-BR&page=1`, this.obterHeaderAutorizacao()
    ).pipe(
        map((obj: any) => this.mapearListaFilmes(obj.results)));
  }

  private mapearListaFilmes(filmes: any): Filme[] {
    console.log("===>", filmes.map((x: any) => x.poster_path))
